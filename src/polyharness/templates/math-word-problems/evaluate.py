"""Evaluator for the math-word-problems example.
...
def evaluate(candidate_dir: Path, task_file: Path | None = None) -> dict:
    ...
    for i, case in enumerate(test_cases):
        ...
        try:
            prediction = float(harness.solve(question))
            # Allow small floating point tolerance
            is_correct = math.isclose(prediction, expected, rel_tol=1e-6, abs_tol=1e-9)
